...
namespace {
    void ensureCacheTxtInitialized(const std::filesystem::path& cachePath) {
        ...
        const auto defaultOutputPath = MyPath::getHomePath() / "Desktop" / "MusicCatOutput";
...
std::filesystem::path MyPath::getProjectPath() {
    return std::filesystem::current_path();
}

std::filesystem::path MyPath::getHomePath() {
...
std::filesystem::path MyPath::getCachePath() {
    if (isTestingInClion) {
...
std::filesystem::path MyPath::getCacheTxtPath() {
...
std::filesystem::path MyPath::getLogPath() {
    if (isTestingInClion) {
